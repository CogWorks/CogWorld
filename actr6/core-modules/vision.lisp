...
(defmethod featlis-to-focus ((vis-mod vision-module) loc feat-lis)
  (let* ((best-feature 
          (find-best-feature vis-mod feat-lis (if (chunk-p-fct (chunk-visicon-entry loc)) (chunk-visicon-entry loc) loc)))
         (dmo-lis (featlis-to-chunks vis-mod (xy-loc (gethash best-feature (visicon vis-mod)))
                                     (feat-match-xy feat-lis (xy-loc (gethash best-feature (visicon vis-mod))))))
...
(defmethod determine-focus-dmo ((vis-mod vision-module) (dmo-lis list) feat )
  ...
  (aif (member feat dmo-lis :key #'chunk-visicon-entry)
       (first it)
      ...
      (if (chunk-type-subtype-p-fct (chunk-chunk-type-fct feat) 'char-primitive)
        ...
        (let ((obj (vis-loc-to-obj (aif (chunk-visual-object feat)
          ...
          (if (and (chunk-p-fct obj)
              ...
              (progn
                (setf (chunk-visicon-entry obj) feat) ;(chunk-visicon-entry feat))
...
(defmethod find-location ((vis-mod vision-module) chunk-spec)
  ...
  (let ((loc (awhen (find-current-locs-with-spec vis-mod chunk-spec)
    ...
    (when (and loc (auto-attend vis-mod))
      (schedule-event-relative .05 'move-attention :params (list :location loc) :destination :vision :output 'medium
                               :module :vision :details (concatenate 'string "Move-attention " (symbol-name loc))))
    loc))
