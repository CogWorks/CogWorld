              ...
              (t ;; it's a valid request
               ...
               (if (eq 'reset (third (car recent)))
                 ...
                 (cond ((or (and (eq t (third (car recent)))   ;; = request t
                        ...
                        (when (dm-act dm)
                          (model-output "Only recently retrieved chunks: ~s"
                                        chunk-list)))
                       (t
    ...
    (let ((best-val nil)
      ...
      (cond ((consp return-val)
            ...
            ((or (null best) 
             ...
             (when (and (dm-act dm) (null best))
               (model-output 
                "No matching chunk found retrieval failure"))
             
             (when (and (dm-act dm) best)
               (model-output 
...
(define-module-fct 'declarative 
  ...
  (list (define-parameter :esc :owner nil)
        ...
        (define-parameter :act :valid-test #'tornil :default-value nil
          :warning "T or nil" :documentation "Activation Trace")
                   ...
                   (dolist (k (chunk-spec-slot-spec request))
                     (when (and (or (eq (car k) '=)
                       ...
                       (let* ((sim (chunks-similarity dm 
                         ...
                         (when (dm-act dm)
                           (model-output "  effective similarity value is ~f"
                                         sim-dif))
    ...
    (cond ((numberp noise)
          ...
          (t
           ...
           (when (dm-act dm)
             ...
             (model-output "Adding permanent noise ~f" 
...
(defun compute-references (dm n references creation-time minus-decay)
  ...
  (when (dm-act dm)
    ...
    (model-output "  creation time: ~f decay: ~f  Optimized-learning: ~s"
                  creation-time (- minus-decay) (dm-ol dm)))
