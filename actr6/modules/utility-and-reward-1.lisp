...
(defun linear-update-utility (production reward)
  (let ((old (production-u production))
        (alpha (car (no-output (sgp :alpha)))))
        ...
        (if (and (utility-esc u) (utility-ul u))
            (progn 
              (dolist (p-t (utility-history u))
                (linear-update-utility (car p-t) (+ c (cdr p-t))))
...
(defun update-utility-for-compiled-production (p3 p1 p2)
  (declare (ignore p2))
  (when (car (no-output (sgp :ul)))
    (linear-update-utility p3 (production-u p1))))
