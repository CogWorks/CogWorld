...
(define-model paired
(sgp :v t :esc t :bll .5 :ans .25 :act t :rt -2 :lf .4)
(sgp :seed (200 4))


(chunk-type assoc state)
(chunk-type pair probe study-item)

(add-dm (goal isa assoc state read-probe))

(p assoc-read-probe
  =goal> isa assoc state read-probe
  =visual-location> isa visual-location
  ?visual> state free
==>
  +visual> isa move-attention 
   screen-pos =visual-location
  =goal> state attend-probe
)


(P assoc-attend
 =goal> isa assoc state attend-probe
 =visual> isa visual-object
  value =val
==>
 +imaginal> isa pair probe =val
 =goal> state check
 +retrieval> isa pair probe =val)

(p assoc-fail
 =goal> isa assoc state check
...
(p assoc-success
 ...
 ?manual> state free
==>
 !output! (pair =probe =val)
 +manual> isa press-key
  key =val
 =goal> state read-study-item
 +visual> isa clear)

(p assoc-read-study-item
  =goal> isa assoc state read-study-item
  =visual-location> isa visual-location
  ?visual> state free
==>
  +visual> isa move-attention 
   screen-pos =visual-location
  =goal> state attend-study-item
)
(P assoc-attend-study-item
 =goal> isa assoc state attend-study-item
 =visual> isa visual-object
  value =val
 =imaginal> isa pair
  study-item nil
==>
 =imaginal> study-item =val
 -imaginal>
 =goal> state read-probe)
