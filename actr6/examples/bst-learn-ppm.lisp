...
(defun collect-data (n)
   (let ((result (make-list (length *bst-stimuli*) :initial-element 0))
         (p-values (list '(force-over 0) '(force-under 0))))
     (dotimes (i n result)
...
(defun compute-difference ()
  (let ((chunk (buffer-read 'imaginal)))
    (schedule-mod-buffer-chunk  'imaginal `(difference ,(abs (- (chunk-slot-value-fct chunk 'length)
...
(sgp :v nil :esc t :egs 3 :show-focus t :trace-detail medium :ul t :ncnar nil :ppm 37 :sim-hook number-sims)
...
(p check-for-done
  ...
  ;; Don't want to miss being done or expect to be done
  ;; due to a partial match to 0
