...
proc create_brain_post_view {win} {
    ...
    set voxel_shades ""
     ...
     scale $win.s -orient horizontal -command "redraw_brain $win" -from 0 -to [expr [llength $voxel_shades]-1] -label "Scan #" -tickinterval [expr [llength $voxel_shades ]-1]
...
global gvar
     ...
     foreach {color} $gvar($win,colors) {
       ...
       if {$color == "none"} {
         $gvar($win,canv) create text 70  $y -text "---" -fill "#ccc" -anchor w -tag values
       } else {
         $gvar($win,canv) create text 70  $y -text [format "%1.3f" $color] -fill "#ccc" -anchor w -tag values
       } 
       incr y 40
     }  
 }
...
set voxel_colors [list "#f00" "#090" "#00f" "#aa0" "#0aa" "#a0a" "#0e0" "#36a"]
 ...
 proc main_bold_brains {win title init} {
     ...
     $gvar($win,canv) create text 10  200 -text "manual" -fill [lindex $voxel_colors 0] -anchor w
     $gvar($win,canv) create text 10  240 -text "goal" -fill [lindex $voxel_colors 1] -anchor w
     $gvar($win,canv) create text 10  280 -text "vocal" -fill [lindex $voxel_colors 2] -anchor w
     $gvar($win,canv) create text 10  320 -text "imaginal" -fill [lindex $voxel_colors 3] -anchor w
     $gvar($win,canv) create text 10  360 -text "retrieval" -fill [lindex $voxel_colors 4] -anchor w
     $gvar($win,canv) create text 10  400 -text "procedural" -fill [lindex $voxel_colors 5] -anchor w
     $gvar($win,canv) create text 10  440 -text "aural" -fill [lindex $voxel_colors 6] -anchor w
     $gvar($win,canv) create text 10  480 -text "visual" -fill [lindex $voxel_colors 7] -anchor w

     $gvar($win,canv) create text 10  10 -text "0.0" -fill "#ccc" -anchor w
     $gvar($win,canv) create text 10  170 -text "1.0" -fill "#ccc" -anchor w
