...
proc make_buffer_viewer {} {
  ...
  bind $list_box <<ListboxSelect>> "select_buffer %W $text_box"
...
proc select_buffer {listwin target_win} {
  if [valid_handler_name $target_win] {
    ...
    if {[llength $selections] != 0} {
      ...
      wm title [winfo parent [winfo parent $listwin]] "$buffer Buffer viewer"

    } else {
      send_environment_cmd "update [get_handler_name $target_win] (lambda (x) (declare (ignore x)))"

      wm title [winfo parent [winfo parent $listwin]] "Buffers viewer"
    }
  }
}
