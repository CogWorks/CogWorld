...
proc make_buffer_status_viewer {} {
  ...
  send_environment_cmd \
    "create list-box-handler $list_box $list_box buffer-list ()"
  ...
  send_environment_cmd \
    "create text-output-handler $text_box $text_box (lambda (x) (declare (ignore x))) (post)"
  ...
  bind $list_box <<ListboxSelect>> "select_buffer_status %W $text_box"
...
proc select_buffer_status {listwin target_win} {
  if [valid_handler_name $target_win] {
    ...
    if {[llength $selections] != 0} {
      ...
      wm title [winfo parent [winfo parent $listwin]] "$buffer Buffer Status viewer"

    } else {
      send_environment_cmd "update [get_handler_name $target_win] (lambda (x) (declare (ignore x)))"

      wm title [winfo parent [winfo parent $listwin]] "Buffers Status viewer"
    }
  }
}
