...
proc display_vert_data {win level} {
  ...
  if {$min_res < 1 || $max_res < 1 || $min_y >= $max_y} {
    ...
    while {$y <= $size} {
      ...
      $win.frame.canvas create text 0 $y -text [expr 0.001 * $y] -anchor w -font graphic_trace_font -tag [list trace_items scale_items]
      incr y 50
    }
...
# user has restricted the size...
 ...
 foreach x $result {
    ...
    while {$y <= $max_y} {
      ...
      $win.frame.canvas create text 0 [expr $y - $min_y] -text [expr 0.001 * $y] -anchor w -font graphic_trace_font -tag [list trace_items scale_items]
      incr y 50
    }
