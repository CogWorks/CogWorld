             ...
             (let ((constants nil)
               ...
               (dolist (buffer-name (production-lhs-buffers production))
                 (let ((bn buffer-name)  ;; closure voodoo
                   ...
                   (if (searchable-buffer buffer-name)
                       (progn
                         (if (find buffer-name searched-buffers)
                             (progn
                               (print-warning "Search buffer ~s is specified in more than one condition." buffer-name)
                               (print-warning "No production defined for ~s." (production-text production))
                               (return-from create-production nil))
                           (push buffer-name searched-buffers))
                            ...
                            (let ((search-specs (list (make-cr-condition :type 'isa :buffer buffer :bi bi :value ct))))
                              ...
                              (dolist (spec (chunk-spec-slot-spec constant))
                                ...
                                (case (car spec)
                                  ...
                                  (- (push-last (make-cr-condition :type 'test-slot :buffer buffer :bi bi :value (third spec) :test 'chunk-slot-equal :slot (second spec) :si (get-slot-index ct (second spec)) :result nil) search-specs))))
                              ...
                              (dolist (v var-spec-list)
                                (let ((var (third v))
                                  ...
                                  (push-last (make-cr-condition :type 'test-slot :buffer buffer :bi bi :value nil :test 'chunk-slot-equal :slot binding-slot :si (get-slot-index ct binding-slot) :result nil) search-specs)
                              ...
                              (dolist (v var2-spec-list)  ;; for a search buffer this is essentially the same as the 
                                ...
                                (when (chunk-spec-variable-p (third v))
                                  (let ((var (third v))  ;; binding the variable from the search
                                    ...
                                    (push-last (make-cr-condition :type 'test-var-slot :buffer buffer :bi bi :value nil :test 'chunk-slot-equal :slot (second v) :result nil) search-specs)
                              ...
                              (dolist (spec others-spec-list) ;; slots that contained variables which aren't bindings
                                (let* ((type (if (chunk-spec-variable-p (second spec)) 'test-var-slot 'test-slot))
                                       (condition 
                                        (case (car spec) 
                                          (=  (make-cr-condition :type type :buffer buffer :bi bi :value (third spec) :slot (second spec) :si (when (eq type 'test-slot) (get-slot-index ct (second spec))) :test 'chunk-slot-equal :result t))
                                          ...
                                          (>= (make-cr-condition :type type :buffer buffer :bi bi :value (third spec) :slot (second spec) :si (when (eq type 'test-slot) (get-slot-index ct (second spec))) :test 'safe>= :result t))
                                          (<= (make-cr-condition :type type :buffer buffer :bi bi :value (third spec) :slot (second spec) :si (when (eq type 'test-slot) (get-slot-index ct (second spec))) :test 'safe<= :result t))                                          
                                          (-  (make-cr-condition :type type :buffer buffer :bi bi :value (third spec) :slot (second spec) :si (when (eq type 'test-slot) (get-slot-index ct (second spec))) :test 'chunk-slot-equal :result nil)))))
                              ...
                              (if (chunk-spec-variable-p (second spec)) 
                                  (case (car spec)  ;; nothing implicit is known nor is a slot index available
                                    ...
                                    (<= (if (and (chunk-spec-variable-p (third spec)) (or (eq 'search (gethash (third spec) var-table)) (eq 'search-slot (gethash (third spec) var-table))))
                                          ...
                                          (push-last (make-cr-condition :type 'test-var-slot :buffer buffer :bi bi :value (third spec) :slot (second spec) :test 'safe<= :result t) others)))
                                    (- (if (and (chunk-spec-variable-p (third spec)) (or (eq 'search (gethash (third spec) var-table)) (eq 'search-slot (gethash (third spec) var-table))))
                                           (push-last (make-cr-condition :type 'test-var-slot :buffer buffer :bi bi :value (third spec) :test 'chunk-slot-equal :slot (second spec) :result nil) search-other)
                                ...
                                (case (car spec) ; treat it just like a normal
                                  ...
                                  (- 
                                   ...
                                   (if (or (eq 'search (gethash (third spec) var-table)) (eq 'search-slot (gethash (third spec) var-table)))
                                       (push-last (make-cr-condition :type 'test-slot :buffer buffer :bi bi :value (third spec) :test 'chunk-slot-equal :slot (second spec) :si (get-slot-index ct (second spec)) :result nil) search-other)
                                     (push-last (make-cr-condition :type 'test-slot :buffer buffer :bi bi :value (third spec) :test 'chunk-slot-equal :slot (second spec) :si (get-slot-index ct (second spec)) :result nil) others))))))))))
                     (#\?
                      (let ((constant-queries (third c))
                        ...
                        (dolist (q variable-queries)
                          (push-last (make-cr-condition :type 'query :buffer buffer :slot (second q) :value (third q) :result (eq '= (first q))) search-other))
