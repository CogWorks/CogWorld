    ...
    (when c
      ...
      (when (update-chunks-on-the-fly) 
        (when (chunk-back-links chunk-name)
          (model-warning "Chunk ~s is being deleted but is still used as a slot value in other chunks." chunk-name))
        
        (when (chunk-back-links tn)
          (model-warning "Chunk ~s is being deleted but it's true name ~s is still used as a slot value in other chunks." chunk-name tn))
