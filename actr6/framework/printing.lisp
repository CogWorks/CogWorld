...
(defun filter-test (event)
  (let ((module (get-module printing-module)))
    (and module 
         (or (and (eq (printing-module-detail module) 'low)
                  (eq (evt-output event) 'low))
             (and (eq (printing-module-detail module) 'medium)
                  (not (eq (evt-output event) t)))
             (eq (printing-module-detail module) 'high))
