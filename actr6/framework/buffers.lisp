    ...
    (dolist (buffer-def buffer-list res)
      (cond ((or (atom buffer-def)
             ...
             (cond ((buffer-exists buffer-def)
                    (print-warning 
                     "Buffer name ~S already used, cannot reuse it.")
                   ...
                   (t
                    (push (make-act-r-buffer 
                           ...
                           :parameter-name 
                           (read-from-string 
                            (format nil ":~S-activation" buffer-def)))
                          res))))
            ...
            ((<= (length buffer-def) 5)
             (let (param-name param-default requests queries print-status)
               (if (and (second buffer-def) (listp (second buffer-def)))
                   (cond ((or 
                          ...
                          (print-warning 
                           "Invalid buffer specification: ~S" buffer-def)
                          (return-from parse-buffers :error))
                         (t
                 ...
                 (cond ((keywordp (second buffer-def))
                       ...
                       ((null (second buffer-def))
                        (setf param-name
                          (read-from-string 
                           (format nil ":~S-activation" (first buffer-def)))))
                       (t
                        (print-warning 
                         "Invalid buffer specification: ~S" buffer-def)
                        (return-from parse-buffers :error))))
  ...
  (dolist (buffer buffers)
    ...
    (install-parameters 'buffer-params 
                        (list (define-parameter 
                                  (act-r-buffer-parameter-name buffer)
                                  :owner t
                                :valid-test #'numberp
                                :default-value 
                                (if (null (act-r-buffer-spread buffer))
                                    0
                                  (act-r-buffer-spread buffer))
                                :warning "a number"
                                :documentation 
                                (format nil 
                                    "source spread for the ~S buffer" 
...
(defun buffer-instance (buffer-name)
  (verify-current-mp  
   "buffer-instance called with no current meta-process."
   (verify-current-model
    "buffer-instance called with no current model."
        ...
        (let* ((buffer (buffer-instance buffer-name)))
          (if buffer
              (let ((chunk (act-r-buffer-chunk buffer)))
                (command-output "~S: ~S ~@[[~s]~]" buffer-name chunk
   ...
   (verify-current-model
    ...
    (let ((buffer (buffer-instance buffer-name)))
      (if buffer 
        ...
        (print-warning "Buffer-read called with an invalid buffer name ~S" 
                       buffer-name))))))


(defun schedule-buffer-read (buffer-name time-delta 
                             &key (module :none) (priority 0)
                             (output t))
  (verify-current-mp  
   ...
   (verify-current-model
    ...
    (let ((buffer (buffer-instance buffer-name)))
      (cond ((null buffer)
             (print-warning 
              "schedule-buffer-read called with an invalid buffer name ~S"
              buffer-name))
            ((not (numberp time-delta))
             (print-warning 
              "schedule-buffer-read called with a non-number time-delta: ~S"
              time-delta))
            ((and (not (numberp priority)) (not (eq priority :max)) 
                  (not (eq priority :min)))
             (print-warning 
              "schedule-buffer-read called with an invalid priority ~S" 
              priority))
            (t
  ...
  (verify-current-mp  
   ...
   (verify-current-model
    ...
    (let ((buffer (buffer-instance buffer-name)))
      (cond ((null buffer)
             (print-warning 
              "buffer-read-report called with an invalid buffer name ~S" 
              buffer-name))
            (t
  ...
  (verify-current-mp  
   ...
   (verify-current-model
    ...
    (let ((buffer (buffer-instance buffer-name)))
      (cond ((null buffer)
             (print-warning 
              "query-buffer called with an invalid buffer name ~S" 
              buffer-name))
            ((not (every #'(lambda (x)
                             (member (car x) (act-r-buffer-queries buffer)))
                         queries-list))
             (print-warning 
              "Invalid query-buffer ~S.  Available queries to buffer ~S are ~S."
                      ...
                      (case (cdar queries) 
                        ...
                        (unrequested
                         (unless (and (act-r-buffer-chunk buffer)
                           ...
                           (return-from query-buffer nil)))
                        (t
                         (model-warning "Unknown buffer query ~S"
                                        (cdar queries))
                         (return-from query-buffer nil))))
                     (t
                      (unless (query-module module buffer-name 
                                            (caar queries) (cdar queries))
                   (return-from query-buffer nil)))))))))))
                                              

(defun schedule-query-buffer (buffer-name queries-list time-delta 
                                   &key (module :none) (priority 0)
                                   (output t))
  (verify-current-mp  
   ...
   (verify-current-model
    ...
    (let ((buffer (buffer-instance buffer-name)))
      (cond ((null buffer)
             (print-warning 
              "schedule-query-buffer called with an invalid buffer name ~S" 
              buffer-name))
            ((not (numberp time-delta))
             (print-warning 
              "schedule-query-buffer called with non-nimber time-delta: ~S" 
              time-delta))
            ((and (not (numberp priority)) (not (eq priority :max)) 
                  (not (eq priority :min)))
             (print-warning 
              "schedule-query-buffer called with an invalid priority ~S" 
              priority))
            (t
  ...
  (verify-current-mp  
   ...
   (verify-current-model
    ...
    (let ((buffer (buffer-instance buffer-name)))
      (cond ((null buffer)
             (print-warning 
              "query-buffer-report called with an invalid buffer name ~S" 
              buffer-name))
            ((not (every #'(lambda (x)
                             (member (car x) (act-r-buffer-queries buffer)))
                         queries-list))
  ...
  (verify-current-mp  
   ...
   (verify-current-model
    ...
    (let ((buffer (buffer-instance buffer-name)))
      (cond ((null buffer)
             (print-warning 
              "schedule-clear-buffer called with an invalid buffer name ~S" 
              buffer-name))
            ((not (numberp time-delta))
             (print-warning 
              "schedule-clear-buffer called with a non-number time-delta: ~S"
              time-delta))
            ((and (not (numberp priority)) (not (eq priority :max)) 
                  (not (eq priority :min)))
             (print-warning 
              "schedule-clear-buffer called with an invalid priority ~S" 
              priority))
            (t
  ...
  (verify-current-mp  
   ...
   (verify-current-model
    ...
    (let ((buffer (buffer-instance buffer-name)))
      (cond ((null buffer)
             (print-warning 
              "schedule-set-buffer-chunk called with an invalid buffer name ~S" 
              buffer-name))
            ((null (get-chunk chunk-name))
             (print-warning 
              "schedule-set-buffer-chunk called with an invalid chunk name ~S" 
              chunk-name))
            ((not (numberp time-delta))
             (print-warning 
              "schedule-set-buffer-chunk called with time-delta that is not a number: ~S"
              time-delta))
            ((and (not (numberp priority)) (not (eq priority :max)) 
                  (not (eq priority :min)))
             (print-warning 
              "schedule-set-buffer-chunk called with an invalid priority ~S" priority))
            (t
...
(defun overwrite-buffer-chunk (buffer-name chunk-name &key (requested nil))
  "Also forces a copy of the chunk..."
  (verify-current-mp  
   ...
   (verify-current-model
    ...
    (let ((buffer (buffer-instance buffer-name)))
      (cond ((null buffer)
             (print-warning 
              "overwrite-buffer-chunk called with an invalid buffer name ~S" 
              buffer-name))
            ((null (get-chunk chunk-name))
             (print-warning 
              "overwrite-buffer-chunk called with an invalid chunk name ~S"
              chunk-name))
            (t
             (setf (act-r-buffer-requested buffer) requested)
             (let ((copy-name (copy-chunk-fct chunk-name)))
...
(defun schedule-overwrite-buffer-chunk (buffer-name chunk-name time-delta 
                                              &key (module :none) (priority 0)
                                                    (output 'low)
                                                    (requested nil))
  (verify-current-mp  
   ...
   (verify-current-model
    ...
    (let ((buffer (buffer-instance buffer-name)))
      (cond ((null buffer)
             (print-warning 
              "schedule-overwrite-buffer-chunk called with an invalid buffer name ~S"
              buffer-name))
            ((null (get-chunk chunk-name))
             (print-warning 
              "schedule-overwrite-buffer-chunk called with an invalid chunk name ~S" 
              chunk-name))
            ((not (numberp time-delta))
             (print-warning 
              "schedule-overwrite-buffer-chunk called with a non-number time-delta: ~S"
              time-delta))
            ((and (not (numberp priority)) (not (eq priority :max)) 
                  (not (eq priority :min)))
             (print-warning 
              "schedule-overwrite-buffer-chunk called with an invalid priority ~S" 
              priority))
            (t
  ...
  (verify-current-mp  
   ...
   (verify-current-model
    ...
    (let ((buffer (buffer-instance buffer-name)))
      (cond ((null buffer)
             (print-warning 
              "module-request called with an invalid buffer name ~S" 
              buffer-name))
            ((null (act-r-chunk-spec-p chunk-spec))
             (print-warning 
              "module-request called with an invalid chunk-spec ~S"
              chunk-spec))
            (t
             (request-module (act-r-buffer-module buffer) buffer-name 
                             chunk-spec)))))))


(defun schedule-module-request (buffer-name chunk-spec time-delta 
                                &key (module :none) (priority 0)
                                (output 'medium) (details nil))
  ...
  (verify-current-mp  
   ...
   (verify-current-model
    ...
    (let ((buffer (buffer-instance buffer-name)))
      (cond ((null buffer)
             (print-warning 
              "schedule-module-request called with an invalid buffer name ~S"
              buffer-name))
            ((null (act-r-chunk-spec-p chunk-spec))
             (print-warning 
              "schedule-module-request called with an invalid chunk-spec ~S" 
              chunk-spec))
            ((not (numberp time-delta))
             (print-warning 
              "schedule-module-request called with a non-number time-delta: ~S"
              time-delta))
            ((and (not (numberp priority)) (not (eq priority :max)) 
                  (not (eq priority :min)))
             (print-warning 
              "schedule-module-request called with an invalid priority ~S"
              priority))
            (t
  ...
  (verify-current-mp  
   ...
   (verify-current-model
    ...
    (let ((buffer (buffer-instance buffer-name)))
      (cond ((null buffer)
             (print-warning 
              "module-mod-request called with an invalid buffer name ~S" 
              buffer-name))
            ((null (act-r-buffer-chunk buffer))
             (print-warning 
              "module-mod-request called with no chunk in buffer ~s"
              buffer-name))
            ((and verify
                  (null (valid-chunk-modification 
                         (act-r-buffer-chunk buffer) modification)))
             (print-warning 
              "module-mod-request called with an invalid modification ~S"
              modification))
            (t
             (buffer-mod-module 
              (act-r-buffer-module buffer) buffer-name modification)))))))


(defun schedule-module-mod-request (buffer-name modification time-delta 
                                &key (module :none) (priority 0)
                                                (output 'medium) (verify t))
  (verify-current-mp  
   ...
   (verify-current-model
    ...
    (let ((buffer (buffer-instance buffer-name)))
      (cond ((null buffer)
             (print-warning 
              "schedule-module-mod-request called with invalid buffer name ~S"
              buffer-name))
            ((null (act-r-buffer-chunk buffer))
             (print-warning 
              "schedule-module-mod-request called with no chunk in buffer ~s"
              buffer-name))
            ((and verify
                  (null (valid-chunk-modification 
                         (act-r-buffer-chunk buffer) modification)))
             (print-warning 
              "schedule-module-mod-request called with an invalid modification ~S"
              modification))
            ((not (numberp time-delta))
             (print-warning 
              "~s called with a non-number time-delta: ~S" 
              'schedule-module-mod-request time-delta))
            ((and (not (numberp priority)) (not (eq priority :max)) 
                  (not (eq priority :min)))
             (print-warning 
              "schedule-module-mod-request called with an invalid priority ~S"
              priority))
            (t
             (schedule-event-relative time-delta  'module-mod-request 
                                      ...
                                      :details
                                      (concatenate 'string
                                        (symbol-name 'module-mod-request)
                                        " "
                                        (symbol-name buffer-name))
  ...
  (verify-current-mp  
   ...
   (verify-current-model
    ...
    (let ((buffer (buffer-instance buffer-name)))
      (cond ((null buffer)
             (print-warning 
              "mod-buffer-chunk called with an invalid buffer name ~S" 
              buffer-name))
            ((null (act-r-buffer-chunk buffer))
             (print-warning 
              "mod-buffer-chunk called with no chunk in buffer ~s" 
              buffer-name))
            ((null (valid-chunk-modification 
                    (act-r-buffer-chunk buffer) modifications))
             (print-warning 
              "mod-buffer-chunk called with an invalid modification ~S" 
              modifications))
            (t 
             (fast-mod-chunk-fct (act-r-buffer-chunk buffer) modifications)))))))
            

(defun schedule-mod-buffer-chunk (buffer-name modifications time-delta 
                                &key (module :none) (priority 0)
                                (output 'low))
  (verify-current-mp  
   ...
   (verify-current-model
    ...
    (let ((buffer (buffer-instance buffer-name)))
      (cond ((null buffer)
             (print-warning 
              "schedule-mod-buffer-chunk called with an invalid buffer name ~S"
              buffer-name))
            ((null (act-r-buffer-chunk buffer))
             (print-warning 
              "schedule-mod-buffer-chunk called with no chunk in buffer ~s" 
              buffer-name))
            ((null (valid-chunk-modification 
                    (act-r-buffer-chunk buffer) modifications))
             (print-warning 
              "schedule-mod-buffer-chunk called with an invalid modification ~S"
              modifications))
            ((not (numberp time-delta))
             (print-warning 
              "schedule-mod-buffer-chunk called with non-number time-delta: ~S"
              time-delta))
            ((and (not (numberp priority)) (not (eq priority :max)) 
                  (not (eq priority :min)))
             (print-warning 
              "schedule-mod-buffer-chunk called with an invalid priority ~S"
              priority))
            (t
  ...
  (verify-current-mp  
   ...
   (verify-current-model
    ...
    (let ((buffer (buffer-instance buffer-name)))
      (cond ((null buffer)
             (print-warning 
              "buffer-spread called with an invalid buffer name ~S" 
              buffer-name))
            (t
             (car (no-output 
                   (sgp-fct 
                    (list (act-r-buffer-parameter-name buffer)))))))))))
...
(defun buffers-module-name (buffer-name)
  (let ((buffer (buffer-instance buffer-name)))
      (cond ((null buffer)
             (print-warning 
              "invalid buffer name ~S" buffer-name))
            (t
             (let ((module (act-r-buffer-module buffer)))
                   (if module
                       module
                     (print-warning 
                      "Could not find a module for buffer ~S"
                      buffer-name)))))))
